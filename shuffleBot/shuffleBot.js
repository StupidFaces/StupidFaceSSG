...
const shuffleResultIds = glob.sync(`${module.path}/../src/shuffles/shuffleResult-*`).map((shuffleResult) => shuffleResult.split('-')[1].split('.')[0]);
...
for (const csvFilePath of filesToShuffle) {
    ...
    let fileLines = fs.readFileSync(csvFilePath)
        .toString()
        .split('\n');
    ...
    for (const line of fileLines) {
        ...
        console.log('LINE_ARRAY:', lineArray)
